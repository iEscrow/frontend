...
import { getUserProfile } from "@/app/services/users/getUser";
  ...
  return (
    <div className="flex flex-col gap-10 min-h-screen pb-10 md:px-20 px-5">
      ...
      <div className="md:w-[50vw] flex flex-col  gap-5">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="md:flex gap-5">
            ...
            <div className="max-md:text-sm">
              <p>
                <span className="text-light-blue">Nombre de usuario: </span>{" "}
                {userProfile[0].username}
              </p>
              <p>
                <span className="text-light-blue">miembro activo desde: </span>{" "}
                {formatDate(userProfile[0].createdAt)}
              </p>
            </div>
            <div className="max-md:text-sm">
              <p>
        ...
        {userProfile[0].BankAccounts.length > 0 ? (
          <Cuenta user={userProfile[0]} />
        ) : (
          "No cuentas con ninguna cuenta bancaria asociada."
        )}
        <div className="divider" />
        <h1 className=" text-2xl">Mis wallets</h1>
        {userProfile[0].Wallets.length > 0 ? (
          <Wallet />
        ) : (
          "No cuentas con ninguna wallet asociada."
        )}
      </div>
    </div>
  );
